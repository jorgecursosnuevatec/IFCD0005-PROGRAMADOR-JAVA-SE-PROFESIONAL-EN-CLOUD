...
public class HashMapMapas {
	...
	public static void main(String[] args) {
		...
		final int LIMITE = 10;
		/**
		 * SecureRandom random = new SecureRandom(); String CHAR_LOWER =
		 * "abcdefghijklmnopqrstuvwxyz"; String CHAR_UPPER = CHAR_LOWER.toUpperCase();
		 * String NUMBER = "0123456789"; String DATA_FOR_RANDOM_STRING = CHAR_LOWER +
		 * CHAR_UPPER + NUMBER;
		 * 
		 * Map<Integer, Object> mapa = new HashMap<>();
		 * 
		 * List<Integer> listaEnteros = IntStream.iterate(1, i -> i +
		 * 1).limit(LIMITE).boxed() .collect(Collectors.toList());
		 * 
		 * List<String> listaAleatoria = new ArrayList<>(); int codigoAscii = 0;
		 * StringBuilder cadena = new StringBuilder(); int caracter;
		 * 
		 * 
		 * for (int i = 0; i < LIMITE; i++) { cadena = new StringBuilder();
		 * 
		 ...
		 * 
		 * System.out.println("\nClave->" + par.getKey()); System.out.println("Valor->"
		 * + par.getValue());
		 * 
		 * }
		 * 
